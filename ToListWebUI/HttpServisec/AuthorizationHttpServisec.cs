...
namespace ToListWebUI.HttpServisec
{
    public class AuthorizationHttpServisec
    {
        ...
        //метод для отправки Post запроса на сервер регестрации
        public async Task<string> RegisterUserAsync(UserRegistration model)
        {
            try
            {
                ...
                var response = await _httpClient.PostAsync("https://localhost:55032/api/APIAccount/RegisterAccount", content);
